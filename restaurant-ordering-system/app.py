...
def admin_menu():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, category, name, price, description, available FROM menu_items ORDER BY category, id"
    )
...
def get_pending_orders():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM orders WHERE status IN ('pending', 'received', 'cooking') ORDER BY created_at ASC"
    )
    orders = cursor.fetchall()
    conn.close()
    order_list = [
